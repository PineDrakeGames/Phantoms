...
namespace Ares
{
    ...
    public class BattleInteractor<T1, T2> : ChainEvaluator where T1 : BattleInteractorData<T2> where T2 : ChainableAction
    {
        ...
        public float EvaluatePower(Actor caster, Actor interactorTarget, Actor actionTarget, AbilityAction action, ref float modifier, AbilityMinigame.MinigameResult minigameResult = AbilityMinigame.MinigameResult.SUCCESS)
        {
			...
			float result = 0f;
			string evaluatedString = actionIdentifiers[action] + "_RAW";
            if (!targetEvaluatedActionValues.ContainsKey(evaluatedString))
            {
                ...
                result = action.EvaluatePower(targetEvaluatedActionValues);
                

                targetEvaluatedActionValues.Add(evaluatedString, result);
            }
			else
			{
				result = targetEvaluatedActionValues[evaluatedString];
			}
