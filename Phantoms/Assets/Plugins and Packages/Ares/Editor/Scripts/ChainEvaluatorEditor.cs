    ...
    public abstract class ChainEvaluatorEditor : AresEditor
    {
        ...
        void OnEnable()
        {
            ...
            actionList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                // Line 2
                if (type == ChainEvaluator.ActionType.ClearBuff)
                {
                    rect.y += EditorGUIUtility.singleLineHeight + 2;
                    xOffset = xStart;
                    DrawField(rect, ref xOffset, 120, spClearBuffType);

					DrawLabel(rect, ref xOffset, 60, "Only Temp");
                    DrawField(rect, ref xOffset, 30, spClearBuffTempOnly);

                    ChainableAction.ClearBuffType clearBuffType = (ChainableAction.ClearBuffType)spClearBuffType.enumValueIndex;

                    if ((clearBuffType == ChainableAction.ClearBuffType.Single) || (clearBuffType == ChainableAction.ClearBuffType.SingleNegative) || (clearBuffType == ChainableAction.ClearBuffType.SinglePosotive))
                    {
                        SerializedProperty spStat = element.FindPropertyRelative("stat");
                        int enumIndex = 0;
                ...
                else if (!(type == ChainEvaluator.ActionType.Environment && (ChainableAction.EnvironmentVariableSetType)spSetType.enumValueIndex == ChainableAction.EnvironmentVariableSetType.Unset))
                {
                    rect.y += EditorGUIUtility.singleLineHeight + 2;
                    xOffset = xStart;
