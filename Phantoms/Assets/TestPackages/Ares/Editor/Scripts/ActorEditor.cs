...
//		readonly Color fallbackFieldBackgroundPro = new Color(.3f, .3f, .3f, 1f);
		...
		private void OnEnable(){
			...
			abilityList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
				...
				SerializedProperty spUseMinigame = element.FindPropertyRelative("overrideUseMinigame");
				SerializedProperty spMinigame = element.FindPropertyRelative("minigame");
				...
				xOffset = 0;
				rect.y += 20;
				DrawLabel(rect, ref xOffset, 80, "Use minigame");
