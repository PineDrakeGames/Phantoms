...
namespace Ares
{
    ...
    public class Battle
    {
        ...
        public void ReleaseProgressDelayLock(BattleDelayElement locker)
        {
            ...
            //			Debug.LogError(xxx+" - "+ProgressDelayLocks.Count);
            //TODO: keep an eye on this, make sure everything still works and remove these comments
            //			if(xxx && ProgressDelayLocks.Count == 0){ //This was the final lock, allow for a final delay between turns
            if (xxx && ProgressDelayLocks.Where(l => l == locker).Count() == 0)
            { //This was the final lock, allow for a final delay between turns
        ...
        void HandleTargetInputRandomly(TargetInputNumActors targetInput)
        {
            ...
            if (!targetInput.TargetSelectCallback(candidateTargets))
            { //candidates were rejected for unknown reasons
        ...
        void ContinueWithTargets(Ability ability, Actor[] targets)
        {
            ...
            if (Rules.SelectActorAction == BattleRules.SelectActionMoment.OnRoundStart)
            {
                ...
                { //Ability has become invalid since queueing it.
        ...
        { //non-child actions only
            ...
            foreach (AbilityAction currentAction in actionsToProcess)
            {
                foreach (Actor chosenAbilityTarget in chosenAbilityTargets)
                {
                    ...
                    int result = ProcessChainAction(ability, currentAction, currentActor, actionTarget, power);
        ...
        void ProcessItemActionEffect(Actor[] chosenItemTargets, Item item, ItemAction action)
        { //non-child actions only
            ...
            foreach (ItemAction currentAction in actionsToProcess)
            {
                foreach (Actor chosenItemTarget in chosenItemTargets)
                {
                    ...
                    int result = ProcessChainAction(item, currentAction, currentActor, actionTarget, power);
        ...
        void ProcessAfflictionActionEffect(Actor target, Affliction affliction, AfflictionAction action)
        { //non-child actions only
            ...
            foreach (AfflictionAction currentAction in actionsToProcess)
            {
                ...
                int result = ProcessChainAction(affliction, currentAction, affliction.Afflicter, currentAction.TargetMode == AfflictionAction.TargetType.Afflicted ? target : affliction.Afflicter, power);
        ...
        void EndAfflictionActionEffect(Actor actor, Affliction affliction, AfflictionAction action, System.Action progressAction)
        {
            if (action == affliction.Data.Actions.Where(a => !a.IsChildEffect).Last())
            {
                if (Rules.ProgressAutomatically)
                {
        ...
        int ProcessChainAction(object evaluater, ChainableAction action, Actor caster, Actor target, int power)
        {
            ...
            switch (action.Action)
            {
                case ChainEvaluator.ActionType.Damage:
                    ...
                    return damageDealt;
                case ChainEvaluator.ActionType.Heal: return target.Heal(power);
                case ChainEvaluator.ActionType.Buff: return target.BuffStat(action.Stat, power);
                case ChainEvaluator.ActionType.Cure: return target.Cure(action.Affliction, power);
                case ChainEvaluator.ActionType.Environment: return ModifyEnvironmentVariable(action.EnvironmentVariable, action.EnvironmentVariableSetMode, power, caster);
        ...
        void ProcessAbilityEnd(Actor actor, AbilityResults abilityResults)
        {//recursive
        ...
        void EndPerformItem(Actor actor, ItemResults itemResults)
        { //recursive
            ...
            foreach (AfflictionAction action in affliction.Data.Actions.Where(a => !a.IsChildEffect))
            {
                ...
                switch (hitStatus)
                {
                    case BattleInteractorData.HitStatus.Hit:
                        ...
                        actor.ConfirmAfflictionActionSuccess(affliction, action);
                        ...
                        break;
                    default:
                        actor.ConfirmAfflictionActionFail(affliction, hitStatus, action);
